...
class Node {
    // Constants for default appearance of the nodes
    static SIZE = 20;                     // Diameter of the nodes
    static COLOR = "#FFFFFF";              // Fill color of the nodes
    static STROKE = "rgba(0, 0, 0, 0)";    // Outline color of the nodes
    static TEXTSIZE = 10;                  // Text size of the node values
    static TEXTCOLOR = "#000000";          // Text color of the node values
    static EDGECOLOR = "#000000";          // Color of this node's upper edge
    static EDGETHICKNESS = 2;              // Thickness of this node's upper edge

    // Color-related constants for visualization purposes
    static VISITED = "#0000FF";  // Blue when this node has been visited
    static SUCCESS = "#00FF00";  // Green when this node is added/found
    static FAILURE = "#FF0000";  // Red when value not found in this node

    // Constants controlling the positions of the nodes
    static HORIZONTALSPACING = 15; // Horizontal distance between nodes
    static VERTICALSPACING = 50;   // Vertical distance between nodes

    constructor(graphicsBuffer, parent = null, size = Node.SIZE,
                 color = Node.COLOR, stroke = Node.STROKE,
                 textSize = Node.TEXTSIZE, textColor = Node.TEXTCOLOR,
                 edgeColor = Node.EDGECOLOR, edgeThickness = Node.EDGETHICKNESS) {

        this.value = null;      // The value this node is holding
        this.leftNode = null;   // Reference to left child node
        this.rightNode = null;  // Reference to right child node

        this.graphicsBuffer = graphicsBuffer; // Off-screen buffer to draw on
    ...
    addValue(value) {
        if (!this.isFilled()) {
            ...
            return this.leftSpacing !== initialLeftSpacing ? this.leftNode : shiftedNode;
        } else if (value > this.value) {
            ...
            return this.rightSpacing !== rightSpacing ? this.rightNode : shiftedNode;
        }
    }
    ...
    setCoordinates(x, y) {
        if (this.isFilled()) {
            if (x === undefined && y === undefined) {
                this.x = this.value < this.parent.value
                    ? this.parent.x - this.parent.leftSpacing
                    : this.parent.x + this.parent.rightSpacing;
                this.y = this.parent.y + Node.VERTICALSPACING;
            } else {
                this.x = x;
                this.y = y;
    ...
    search(value) {
        ...
        return value < this.value ? this.leftNode.search(value) : this.rightNode.search(value);
    }

    drawEdge() {
        if (this.hasParent()) {
            this.graphicsBuffer.strokeStyle = this.edgeColor;
            this.graphicsBuffer.lineWidth = this.edgeThickness;
            this.graphicsBuffer.beginPath();
            this.graphicsBuffer.moveTo(this.x, this.y);
            this.graphicsBuffer.lineTo(this.parent.x, this.parent.y);
            this.graphicsBuffer.stroke();
        }
    }

    drawNode() {
        this.graphicsBuffer.fillStyle = this.color;
        this.graphicsBuffer.strokeStyle = this.stroke;
        this.graphicsBuffer.beginPath();
        this.graphicsBuffer.arc(this.x, this.y, this.size / 2, 0, Math.PI * 2);
        this.graphicsBuffer.fill();
        this.graphicsBuffer.stroke();

        this.graphicsBuffer.fillStyle = this.textColor;
        this.graphicsBuffer.font = `${this.textSize}px Arial`;
        this.graphicsBuffer.textAlign = "center";
        this.graphicsBuffer.textBaseline = "middle";
        this.graphicsBuffer.fillText(this.value, this.x, this.y);
    }
